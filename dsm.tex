	...
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item $t > \max{g(x) \rbar x \in K}$, $K_t = K$. %we obtain the entire domain of the image
				\end{itemize}
...
\frame{\frametitle{Discrete Morse Functions}
	...
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				...
				\end{itemize}
				\item $\# \{\beta^{(p+1)} > \alpha \rbar f(\beta) \leq  f(\alpha)\} \leq 1,$
				\item and 
				\item $\# \{\gamma^{(p-1)} < \alpha \rbar f(\gamma) \geq  f(\alpha)\} \leq 1,$
