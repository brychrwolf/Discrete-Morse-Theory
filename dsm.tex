...
\frame{\frametitle{3D Grayscale Digital Images}
	\begin{columns}[T]
		\begin{column}{.66\textwidth}
			\begin{itemize}
				\item $g : D \rightarrow \mathbb{R} $
				\item $D \subset \mathbb{Z}^3 $ is rectangular subset of the discrete lattice: \\
				\[ D = \{(i,j,k)\hspace{1ex}\rvert\hspace{1ex}0 \leq i \leq I,\hspace{1ex} 0 \leq j \leq J,\hspace{1ex} 0 \leq k \leq K\} \]
				\item A point $x \in D$ is called a {\color{MyTeal}voxel} (or pixel in 2D).
				%The algorithms presented in Section 3 apply without adjustment to 2D digital images.
			\end{itemize}
		\end{column}
		\begin{column}{.33\textwidth}
		\end{column}
...
%------------------------------------------------
%TODO: add animated K with colors as each p-cell comes out
\frame{\frametitle{$K$ and \emph{p-cells}}
	\begin{columns}[T]
		\begin{column}{.6\textwidth}
			$K$ is collection of all \emph{p-cells} built from voxels in D, \\
			with $p = \{0, 1, 2, 3\}$
			\begin{itemize}
				\only<1->{
					\item (0-cells) are vertices (i.e. voxels)
				}\only<2->{
					\item (1-cells) are unit edges between voxels %whose coordinates differ by one in a single axis,
				}\only<3->{ 
					\item (2-cells) are unit squares,
				}\only<4->{
					\item (3-cells) are unit cubes
				}
			\end{itemize}
		\end{column}
		\begin{column}{.4\textwidth}
			...
			\only<1>{
				...
				\foreach \x in {-12mm,  8mm, 28mm}
					\draw[tealStyle](\x,-16mm) -- (\x, 24mm);
				\foreach \x in {-20mm,  0mm, 20mm}
					\draw[tealStyle](\x,-20mm) -- (\x, 20mm);
				\foreach \x in {-28mm, -8mm, 12mm}
					\draw[tealStyle](\x,-24mm) -- (\x, 16mm);
				
				\foreach \y in {-16mm,  4mm, 24mm}
					\draw[tealStyle](-12mm,\y) -- ( 28mm,\y);
				\foreach \y in {-20mm,  0mm, 20mm}
					\draw[tealStyle](-20mm,\y) -- ( 20mm,\y);
				\foreach \y in {-24mm, -4mm, 16mm}
					\draw[tealStyle](-28mm,\y) -- ( 12mm,\y);
					
					
					\draw[tealStyle](-12mm,-16mm) -- (-28mm,-24mm);
			}\only<3->{
			}\only<4->{
			}
						

				%\draw[->, indLine] (0mm, 1.5mm) -- (p1.north);
...
%------------------------------------------------
%TODO: add images for lower level cuts
\frame{\frametitle{lower level cuts}
	\begin{columns}[T]
		\begin{column}{.66\textwidth}
			\begin{itemize}
				\item the set of all voxels with grayscale values less than a threshold $t$, is called a {\color{MyTeal}lower level cut}:
				\item $D_t = \{x \in D \rvert g(x) \leq t\}$.
				\item In fact, we will mostly consider level cuts on the cubical complex, where a cell $\alpha$ is included if all its vertices $x \in \alpha$ are below the threshold:
				\item $K_t = \{ \alpha \in K \rvert g(x) \leq t, \forall x \in \alpha\} $
				\item which is equivalent to $D_t$ with standard 6-neighborhood of topology, where diagonals are only considered in the presnece of at least one shared standard voxel.
			\end{itemize}
		\end{column}
		\begin{column}{.33\textwidth}
		\end{column}
...
%------------------------------------------------
\frame{\frametitle{Main Goal of the Paper}
	%\begin{columns}[T]
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				\item Describe how the topology of the lower level cuts changes as the threshold is varied.
				\begin{itemize}
					\item $t < \min{g(x) \rvert x \in K}$, $K_t = \{\varnothing\}$; 
					\item as $t$ increases, cells are added so that for any $t_1 < t_2$, $K_{t_1}$ is a subcomplex of $K_{t_2}$
					\item $t > \max{g(x) \rvert x \in K}$, $K_t = K$. %we obtain the entire domain of the image
				\end{itemize}
...
\frame{\frametitle{Simple Homotopy}
	...
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				...
				\item For a cubical complex $K$ with cell $\beta{(p)}$, we say that has a {\color{MyTeal}free face} $\alpha^{(p-1)}$ if $\alpha$ is a face of $\beta$, and $\alpha$ has no other cofaces.
				\item We call $(\alpha, \beta)$ a {\color{MyTeal}free pair}. 
				\item If we remove a free pair from $K$, we obtain a subcomplex of $K$ that is called an {\color{MyTeal}elementary collapse} of K.
				\item We say that a cubical complex $K$ {\color{MyTeal}collapses} to a subcomplex $K'$ if there is a finite sequence of elementary collapses from $K$ to $K'$. 
				\item If a cubical complex collapses to a point it is {\color{MyTeal}collapsible}.
				\item The inverse of a collapse is an {\color{MyTeal}expansion}. 
				\item We say that two cubical complexes are {\color{MyTeal}simple homotopy equivalent} if there is a sequence of collapses and expansions from one complex to the other. 
				\item An elementary collapse determines a strong deformation retraction. It therefore follows that if two cubical complexes are simple homotopy equivalent, then they are homotopy equivalent. 
			\end{itemize}
	%	\end{column}
	%	\begin{column}{.33\textwidth}
	%	\end{column}
...
\frame{\frametitle{Discrete Morse Functions}
	...
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				\item A function, $f : K \rightarrow \mathbb{R}$ is a {\color{MyTeal}discrete Morse function} if for every $\alpha^{(p)} \in K$, 
				\begin{itemize}
				...
				\end{itemize}
				\item $\# \{\beta^{(p+1)} > \alpha \rvert f(\beta) \leq  f(\alpha)\} \leq 1,$
				\item and 
				\item $\# \{\gamma^{(p-1)} < \alpha \rvert f(\gamma) \geq  f(\alpha)\} \leq 1,$
				\item where $\#$ denotes the number of elements in the set. 
				\item A cell, $\alpha^{(p)}$ is {\color{MyTeal}critical} if all cofaces take strictly greater values and all faces are strictly lower.
			\end{itemize}
	%	\end{column}
	%	\begin{column}{.33\textwidth}
	%	\end{column}
...
\frame{\frametitle{Lower-Star}
	...
	%	\begin{column}{.66\textwidth}
			\begin{itemize}
				...
				\item \[ L(x) = \{\alpha \in K \rvert x \in \alpha \text{ and } g(x) = \max_{y \in \alpha}{g(y)}\}\]
