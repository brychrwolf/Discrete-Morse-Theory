...
\frame{\frametitle{Motivation}
	\begin{itemize}
		...
		\vspace{\baselineskip}
		\item Topological information is independent of geometric measures, however, computational topology require a combinatorial complex (huge) \pause \\
		\vspace{\baselineskip}
		\item A grayscale digital image, which are functions defined on a discrete lattice	cubical Complex, scale as $O(n^3)$, while a Morse complex stores the same topological information.\\
%			Morse Theory
...
\frame{\frametitle{Lower Stars}
	\begin{itemize}
		...
		\item Each subcomplex $K_{t_i}$ differs from each $K_{t_{i-1}}$ only by the single voxel $x_i = g^{-1}(t_i)$ and the 1-, 2- and 3-cells for which $x_i$ is the highest valued voxel.\\
		\item This set of cells in the neighbourhood of $x_i$ is called the {\color{MyTeal}lower star}.
		\[ L(x) = \{\alpha \in K \rbar x \in \alpha \text{ and } g(x) = \max_{y \in \alpha}{g(y)}\}\] \\
		%\item The lower stars of all voxels $x \in D$ form a disjoint partition of $K$.
		\vspace{\baselineskip}
		\item The algorithm \emph{ProcessLowerStars} finds a sequence for adding the cells from $L(x_i)$ to $K_{t_{i-1}}$ to grow it into $K_{t_i}$
...
\frame{\frametitle{Sandstone - Critical Cells}
	\begin{columns}[T]
		...
		\begin{column}{.6\textwidth}
			...
			Critical cells of the discrete Morse function derived from by applying the ProcessLowerStars. \\
...
\frame{\frametitle{Outlook}
	\begin{itemize}
		\item Data Compression and Faster Algorithms
